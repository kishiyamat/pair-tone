...
#### FAQ: OpenJTalk の読みがかなり不自然なとき

OpenJTalk の出力は、アクセント核の位置だけでなく、**語の読みそのもの** や **フレーズの切り方** まで大きく直す必要があることがあります。

> 例: `ミンナ'ガ/キニ'/_シテ'/イタ'/ノワ'/フド'オサンノ/ケエヤクジョオケ'ンオ/_ヒカ_ク'/スル'/ハナシ'ダッタ`
>  
> 修正例: `ミンナ'ガ/キニ_シテ'/イタ'/ノワ'、フド'オサンノ/ケエヤクジョ'オケンオ/_ヒカ_ク'/スルハナシ'ダッタ`

このようなケースでは、次をまとめて見直してください。

- 語の読み自体が合っているか
- `/` を消して、ひとまとまりで読むべき部分を結合すべきか
- `、` を入れて、意味の切れ目を明示したほうが自然か
- 独立しすぎたフレーズを、後続語とつなげたほうが自然か

OpenJTalk を土台にはしますが、**出力に引きずられすぎず、自然な読みになる形を優先**してください。
"""
        )
    st.divider()
